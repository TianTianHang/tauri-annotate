...
import FrameRateCalculator from "./components/FrameRateCalculator";
import SaveRangeControl from "./components/SaveRangeControl";
...
function App() {
  ...
  async function runFinalAnalysis() {
    ...
    try {
      ...
      const responseStr = await invoke<string>("invoke_python", {
        ...
        params: { person_ids: Array.from(idsToSave), frame_skip: frameSkip },
      });
      const response = JSON.parse(responseStr);
      if (response.status === 'success') {
  ...
  const handleIdInputChange = (index: number, value: string) => {
    setSwapState(prev => {
      ...
      newIds[index] = parseInt(value, 10) || 0;
    ...
    <div className="annotation-tool">
      <aside className="sidebar">
        <div className="settings-section">
          <Settings onApply={() => { setPythonReady(true); getNextFrame(); }} />
        </div>
        <hr />
        <ControlPanel
          appPhase={appPhase}
        ...
        />
        <PlaybackSpeedControl fps={fps} setFps={setFps} isPlaying={isPlaying} />
        <hr />
        <FrameRateCalculator
          sourceFps={sourceFps}
          ...
          calculateFrameSkip={calculateFrameSkip}
        />
        <hr />
        <SaveRangeControl
        ...
        <ManualCorrection
          ...
          manualBboxes={manualBboxes}
          setManualBboxes={setManualBboxes}
          swapState={swapState}
          setSwapState={setSwapState}
          handleInitiateSwap={handleInitiateSwap}
          handleIdInputChange={handleIdInputChange}
          handleConfirmSwap={handleConfirmSwap}
          submitManualBbox={submitManualBbox}
        />
        <hr />
        <DetectionList selectedCamId={selectedCamId} currentCamData={currentCamData} />
        <hr />
        <PersonList
          allUniquePersonIds={allUniquePersonIds}
          idsToSave={idsToSave}
          handleToggleIdForSave={handleToggleIdForSave}
        />
      </aside>
      ...
      <main className="main-content">
        ...
        </div>
        {
          ...
          />,[displayFrameData, imageSize.width, imageSize.height, handleMouseDown, handleMouseMove, handleMouseUp])
        }
      </main>
    </div>
  );
